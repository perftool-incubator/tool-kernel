...
"""Post-process turbostat output and emit CDM metrics.

Runs in the kernel tool's data directory (one per profiler instance).
Discovers turbostat-stdout.txt(.xz) and writes to postprocess/.
...
# Columns we extract from the turbostat TSV.
_WANT_COLS = {
    "Time_Of_Day_Seconds",
    "Package", "Die", "L3", "Core", "CPU",
...
def process_turbostat(log_file: str) -> None:
    ...
    for raw_line in fh:
        ...
        if package == "-":
            ...
            # ── Per-CPU row ───────────────────────────────────────────────
            # cpu_field may be decimal or hex (e.g. "3" or "0x3"); normalise.
            try:
